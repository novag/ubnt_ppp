...
static int default_interim_interval = 0;

static option_t Options[] = {
    { "radius-config-file", o_string, &config_file },
    { "avpair", o_special, add_avp },
    { "default-interim", o_int, &default_interim_interval },
		 ...
		 char *message, int message_space)
{
    ...
    if (rstate.acct_interim_interval == 0 && default_interim_interval
	    && default_interim_interval >= 60) {
	rstate.acct_interim_interval = default_interim_interval;
    }
...
radius_acct_stop(void)
{
	...
	av_type = extended_link_stats.long_bytes_out & 0xffffffff;
	rc_avpair_add(&send, PW_ACCT_OUTPUT_OCTETS, &av_type, 0, VENDOR_NONE);

	av_type = extended_link_stats.long_bytes_in & 0xffffffff;
	rc_avpair_add(&send, PW_ACCT_INPUT_OCTETS, &av_type, 0, VENDOR_NONE);

	av_type = extended_link_stats.long_bytes_out >> 32;
	rc_avpair_add(&send, PW_ACCT_OUTPUT_GIGAWORDS, &av_type, 0, VENDOR_NONE);

	av_type = extended_link_stats.long_bytes_in >> 32;
